<template>
  <div>
    <el-dialog :close-on-click-modal="false" :visible.sync="visible" @closed="dialogClose">
      <el-dialog :visible.sync="innerVisible" append-to-body title="选择库存种类" width="40%">
        <div>
          ...
          >
            ...
            <el-table-column align="center" header-align="center" label="id" prop="stockTypeId"></el-table-column>
            <el-table-column align="center" header-align="center" label="种类名" prop="stockTypeName"></el-table-column>
      ...
      <el-row>
        <el-col :span="24">
          ...
          >
            ...
            <el-table-column label="#" prop="stockTypeId"></el-table-column>
            <el-table-column label="库存种类名" prop="stockTypeName"></el-table-column>
            ...
            <el-table-column align="center" fixed="right" header-align="center" label="操作">
              <template slot-scope="scope">
                <el-button size="small" type="text" @click="relationRemove(scope.row.stockTypeId)">移除</el-button>
              </template>
            </el-table-column>
          </el-table>
        </el-col>
  ...
  methods: {
    ...
    batchDeleteRelation (val) {
      ...
      this.dataListSelections.forEach(item => {
        postData.push({stockTypeId: item.stockTypeId, shelfId: this.shelfId})
      })
      this.$http({
        url: this.$http.adornUrl('/stock/shelf/stockType/relation/delete'),
        method: 'post',
    ...
    //移除关联
    relationRemove (stockTypeId) {
      let data = []
      data.push({stockTypeId, shelfId: this.shelfId})
      this.$http({
        url: this.$http.adornUrl('/stock/shelf/stockType/relation/delete'),
        method: 'post',
    ...
    submitAddRealtion () {
      ...
      if (this.innerdataListSelections.length > 0) {
        ...
        this.$http({
          url: this.$http.adornUrl('/stock/shelf/stockType/relation'),
          method: 'post',
          data: this.$http.adornData(postData, false)
        }).then(({data}) => {
          if (data.code == 0) {
    ...
    init (id) {
      ...
      this.$http({
        url: this.$http.adornUrl(
          '/stock/shelf/' + this.shelfId + '/stockType/relation'
        ),
        method: 'get',
    ...
    getDataList () {
      ...
      this.$http({
        url: this.$http.adornUrl(
          '/stock/shelf/' + this.shelfId + '/stockType/norelation'
        ),
        method: 'get',
        params: this.$http.adornParams({
          page: this.pageIndex,
