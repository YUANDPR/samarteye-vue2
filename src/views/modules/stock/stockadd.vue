    ...
    <el-row>
      <el-col :span="24">
        <el-steps :active="step" finish-status="success">
          <el-step title="货物信息"></el-step>
          <el-step title="货物基本信息"></el-step>
      ...
      <el-col v-show="step==0" :span="24">
        <el-card class="box-card" style="width:80%;margin:20px auto">
          <el-form ref="spuBaseForm" :model="spu" :rules="spuBaseInfoRules" label-width="120px">
            ...
            <el-form-item label="货物简图" prop="decript">
              <multi-upload v-model="spu.decript"></multi-upload>
            </el-form-item>

            <el-form-item label="货物详细图集" prop="images">
              <multi-upload v-model="spu.images"></multi-upload>
            </el-form-item>
            <el-form-item>
              <el-button type="success" @click="collectSpuBaseInfo">下一步：设置货物基本信息</el-button>
            </el-form-item>
          </el-form>
        </el-card>
      </el-col>
            ...
            >
              ...
              <el-form ref="form" :model="spu">
                ...
                >
                  ...
                  数量
                  <el-input v-model="dataResp.baseAttrs[gidx][aidx].onestockCount" size="small"
                            style="width:100px"
                  ></el-input>
            ...
            <div slot="header" class="clearfix">
              ...
              <el-form ref="saleform" :model="spu">
                ...
                >
                  <el-input
                    v-show="false"
                    v-model="dataResp.tempSaleAttrs[aidx].stockTypeId"
                    type="hidden"
                  ></el-input>
      ...
      <el-col v-show="step==3" :span="24">
        <el-card class="box-card" style="width:80%;margin:20px auto">
          <el-table :data="spu.skus" style="width: 100%">
            ...
            <el-table-column label="数量" prop="onestockCount">
              <template slot-scope="scope">
                <el-input v-model="scope.row.onestockCount"></el-input>
              </template>
            </el-table-column>
                    ...
                    <el-col :span="24">
                      <el-form-item label="设置叫料权限">
                        ...
                        <!--       <label style="margin-left:15px;">打</label>
                               <el-input-number
                                 style="width:160px"
                                 v-model="scope.row.disonestockCount"
                                 :precision="2"
                                 :max="1"
                                 :min="0"
                                 :step="0.01"
                               ...
                               <label>折</label>
                               <el-checkbox
                                 v-model="scope.row.onestockCountStatus"
                                 :true-label="1"
                                 :false-label="0"
                               >可叠加优惠</el-checkbox> -->
                      </el-form-item>
      ...
      <el-col v-show="step==4" :span="24">
        ...
        <el-card class="result-card" style="width:80%;margin:20px auto">
          <h1>入库成功</h1>
...
export default {
  ...
  data () {
    return {
      ...
      spuBaseInfoRules: {
        spuName: [
          {required: true, message: '请输入货物名字', trigger: 'blur'}
        ],
        spuDescription: [
        ...
        supplierId: [
          {required: false, message: '请选择一个供用商', trigger: 'blur'}
        ],
        decript: [
          {required: false, message: '请上传货物详情图集', trigger: 'blur'}
        ],
        images: [
          {required: false, message: '请上传货物图片集', trigger: 'blur'}
        ],
        weight: [
          {
            type: 'number',
  ...
  methods: {
    ...
    generateSaleAttrs () {
      ...
      this.dataResp.baseAttrs.forEach(item => {
        item.forEach(attr => {
          let {stockTypeId, attrValues, onestockCount} = attr
          //跳过没有录入值的属性
          if (attrValues != '') {
            ...
            this.spu.baseAttrs.push({stockTypeId, attrValues, onestockCount})
          }
        })
      })
    ...
    /**
     ...
     //构造saleAttr信息
     let saleAttrItem = {
     stockTypeId: this.dataResp.tableAttrColumn[index].stockTypeId,
     stockTypeName: this.dataResp.tableAttrColumn[index].stockTypeName,
     attrValue: de
     };
      ...
      if (!this.dataResp.steped[1]) {
        this.$http({
          url: this.$http.adornUrl(
            `/stock/stocktype/sale/list/${this.spu.wlId}`
          ),
          method: 'get',
          params: this.$http.adornParams({
            page: 1,
    ...
    showBaseAttrs () {
      if (!this.dataResp.steped[0]) {
        this.$http({
          url: this.$http.adornUrl(
            `/stock/shelf/${this.spu.wlId}/withStockType`
          ),
          method: 'get',
          params: this.$http.adornParams({})
        }).then(({data}) => {
    ...
    submitSkus () {
      this.$confirm('将要提交入库数据，是否继续?', '提示', {
        ...
        .then(() => {
          this.stock = {
            stockName: this.spu.spuName,
            stockDescription: this.spu.spuDescription,
            wlId: this.spu.wlId,
            weight: this.spu.weight,
            stockImageInfo: this.spu.decript,
            supplierId: this.spu.supplierId,
            oneStock: this.spu.skus
          }
          console.log('~~~~~', JSON.stringify(this.stock))
          this.$http({
            url: this.$http.adornUrl('/stock/stock/save'),
            method: 'post',
            data: this.$http.adornData(this.stock, false)
          }).then(({data}) => {
            if (data.code == 0) {
              this.$message({
                type: 'success',
    ...
    /**
     ...
     //parent上一级索引;onestockCount指针计数

     var point = {};

     var result = [];
     var pIndex = null;
     var temponestockCount = 0;
     var temp = [];

     //根据参数列生成指针对象
     for (var index in list) {
     if (typeof list[index] == "object") {
     point[index] = { parent: pIndex, onestockCount: 0 };
     pIndex = index;
     }
     }
     ...
     while (true) {
     for (var index in list) {
     temponestockCount = point[index]["onestockCount"];
     temp.push(list[index][temponestockCount]);
     }
     ...
     //检查指针最大值问题
     while (true) {
     if (point[index]["onestockCount"] + 1 >= list[index].length) {
     point[index]["onestockCount"] = 0;
