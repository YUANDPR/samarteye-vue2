<template>
  ...
  >
    ...
    >
      ...
      <el-form-item label="供用商logo地址" prop="logo">
        <!-- <el-input v-model="dataForm.logo" placeholder="供用商logo地址"></el-input> -->
...
export default {
  ...
  methods: {
    init (id) {
      this.dataForm.eeId = id || 0
      this.visible = true
      this.$nextTick(() => {
        this.$refs['dataForm'].resetFields()
        if (this.dataForm.eeId) {
          this.$http({
            url: this.$http.adornUrl(
              `/stock/supplier/info/${this.dataForm.eeId}`
            ),
            method: 'get',
    ...
    dataFormSubmit () {
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(
              `/stock/supplier/${!this.dataForm.eeId ? 'save' : 'update'}`
            ),
            method: 'post',
            data: this.$http.adornData({
              eeId: this.dataForm.eeId || undefined,
