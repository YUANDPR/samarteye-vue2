<template>
  ...
  >
    <el-form ref="dataForm" :model="dataForm" :rules="dataRule" label-width="120px">
      ...
      <el-form-item label="库存种类" prop="materialTypeName">
        <el-input v-model="dataForm.materialTypeName" placeholder="库存种类"></el-input>
      </el-form-item>
      <el-form-item label="货物类型" prop="materialTypeType">
        <el-select v-model="dataForm.materialTypeType" placeholder="请选择">
          <el-option :value="1" label="基本货物"></el-option>
          <el-option :value="0" label="货架外货物"></el-option>
        </el-select>
      </el-form-item>
      ...
      <el-form-item v-if="type == 1" label="所属货架" prop="shelfId">
...
export default {
      ...
      if (path && path.length == 3) {
        this.$http({
          url: this.$http.adornUrl(
            `/material/materialtype/list/${path[path.length - 1]}`
          ),
          method: 'get',
  ...
  methods: {
    init (id) {
      ...
      this.dataForm.materialTypeType = this.type
      this.visible = true
      this.$nextTick(() => {
        ...
        if (this.dataForm.materialTypeId) {
          this.$http({
            url: this.$http.adornUrl(
              `/material/materialtype/info/${this.dataForm.materialTypeId}`
            ),
            method: 'get',
            params: this.$http.adornParams()
          }).then(({data}) => {
            if (data && data.code === 0) {
              this.dataForm.materialTypeName = data.data.materialTypeName
              ...
              this.dataForm.materialTypeType = data.data.materialTypeType
    ...
    dataFormSubmit () {
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(
              `/material/materialtype/${!this.dataForm.materialTypeId ? 'save' : 'update'}`
            ),
            method: 'post',
            data: this.$http.adornData({
              materialTypeId: this.dataForm.materialTypeId || undefined,
              materialTypeName: this.dataForm.materialTypeName,
              searchType: this.dataForm.searchType,
              valueType: this.dataForm.valueType,
              icon: this.dataForm.icon,
              valueSelect: this.dataForm.valueSelect.join(';'),
              materialTypeType: this.dataForm.materialTypeType,
